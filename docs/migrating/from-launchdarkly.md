...
> browser SDK loads config over SSE from the delivery API and has no datadir
...
![Two ways to run the migration: free mode pulls LaunchDarkly into a git repo you own and your server-side SDK reads it from disk; cloud mode adds --push, syncing the repo to Quonfig Cloud so every SDK gets instant updates over SSE](/img/docs/migrating/launchdarkly-two-modes.svg)
